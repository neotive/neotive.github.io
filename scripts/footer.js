...
function init() {
    if (ui_main_section.childElementCount > 1 || ui_main_section.querySelector("#footer") === undefined) {
        ...
        const text_terms = config.text.footer_terms;
        const link_terms = config.text.footer_terms_link;
        ...
        if (text_terms !== undefined && link_terms !== undefined) {
            const ui_privacy = document.createElement("a");
            ui_privacy.innerText = text_terms;
            ui_privacy.href = link_terms;
            divider(text_divider, ui_footer);
            ui_footer.appendChild(ui_privacy);
        }
    } else {
